...
from .fibonacci import fib_gen as _fg  # for access
...
def basic(start: int = 1) -> _c.Generator:
    i = start
    while True:
        yield i
        i += 1
...
def root(root: int, start: int = 1) -> _c.Generator:
...
def countdown(start: int) -> _c.Generator:
...
def power(exponent: int) -> _c.Generator:
...
def alphabet(lower: bool = False) -> _c.Generator:
